    ...
    pub async fn init(
        ...
        let transitions = vec![
            (
                ...
                Box::new(|m: &mut Modem| m.start_lte_setup(ca_chain, certificate, private_key))
                    as Box<dyn Fn(&mut Modem) -> Result<(), ModemError> + 'static>,
            ),
            (
                State::MqttOpenConnection,
                Box::new(|m| m.check_network_registration()),
            ),
            (
                State::MqttConnectBroker,
                Box::new(|m| m.mqtt_open_connection()),
            ),
            (State::GetGPSData, Box::new(|m| m.mqtt_connect_broker())),
        ];
        self.modem
            .run_state_machine(State::UploadMqttCert, &transitions)
            .await?;
    ...
    pub async fn mqtt_handler(
        ...
        loop {
            ...
            if let Ok(frame) = can_channel.try_receive() {
                ...
                let can_data = crate::task::can::CanFrame {
                    id: frame.id,
                    len: frame.len,
                    data: frame.data,
                };
