            ...
            LteState::Off => {
                ...
                match modem.modem_init().await {
                    ...
                    Err(e) => {
                        retry_count += 1;
                        error!(
                            "[LTE] Modem initialization failed: {e:?} (Attempt {retry_count}/{MAX_RETRIES})"
                        );
            ...
            LteState::Lte => {
                    ...
                    .await
                {
                    ...
                    Err(e) => {
                        retry_count += 1;
                        error!(
                            "[LTE] LTE initialization failed: {e:?} (Attempt {retry_count}/{MAX_RETRIES})"
                        );
                        if retry_count >= MAX_RETRIES {
            ...
            LteState::Gps => {
                ...
                match modem.gps_init().await {
                    ...
                    Err(e) => {
                        retry_count += 1;
                        error!(
                            "[LTE] GPS initialization failed: {e:?} (Attempt {retry_count}/{MAX_RETRIES})"
                        );
                        if retry_count >= MAX_RETRIES {
                            state = LteState::Error(e);
                            retry_count = 0; // Reset retry count for error state
            ...
            LteState::GetGps => {
                ...
                match modem.get_gps(mqtt_client_id, gps_channel).await {
                    ...
                    Err(e) => {
                        retry_count += 1;
                        error!(
                            "[LTE] GPS data retrieval failed: {e:?} (Attempt {retry_count}/{MAX_RETRIES})"
                        );
                        if retry_count >= MAX_RETRIES {
                            state = LteState::Error(e);
                            retry_count = 0; // Reset retry count for error state
            ...
            LteState::HealthCheck => {
                ...
                match modem.health_check_lte().await {
                    ...
                    Err(e) => {
                        retry_count += 1;
                        error!(
                            "[LTE] HealthCheck failed: {e:?} (Attempt {retry_count}/{MAX_RETRIES})"
                        );
                        if retry_count >= MAX_RETRIES {
                            state = LteState::Error(e);
                            retry_count = 0; // Reset retry count for error state
            ...
            LteState::InitMqttLte => {
                ...
                match modem.init_mqtt_over_lte().await {
                    ...
                    Err(e) => {
                        retry_count += 1;
                        error!(
                            "[LTE] InitMqttLte failed: {e:?} (Attempt {retry_count}/{MAX_RETRIES})"
                        );
                ...
                if let Ok(frame) = can_channel.try_receive() {
                    ...
                    if let Ok(len) = serde_json_core::to_slice(&can_data, &mut buf) {
                        ...
                        if can_payload.push_str(&json).is_err() {
                            retry_count += 1;
                            error!(
                                "[LTE] Payload buffer overflow (Attempt {retry_count}/{MAX_RETRIES})"
                            );
                            if retry_count >= MAX_RETRIES {
                                state = LteState::Error(ModemError::Command);
                                retry_count = 0; // Reset retry count for error state
                            ...
                            if check_result(
                                ...
                                retry_count += 1;
                                error!(
                                    "[LTE] Failed to publish CAN data (Attempt {retry_count}/{MAX_RETRIES})"
                                );
                                if retry_count >= MAX_RETRIES {
                        ...
                        retry_count += 1;
                        error!(
                            "[LTE] Failed to serialize CAN data (Attempt {retry_count}/{MAX_RETRIES})"
                        );
                        if retry_count >= MAX_RETRIES {
                            state = LteState::Error(ModemError::Command);
                            retry_count = 0; // Reset retry count for error state
                    ...
                    if let Ok(len) = serde_json_core::to_slice(&trip_data, &mut buf) {
                        ...
                        if trip_payload.push_str(&json).is_err() {
                            ...
                            if check_result(
                                ...
                                retry_count += 1;
                                error!(
                                    "[LTE] Failed to publish trip data (Attempt {retry_count}/{MAX_RETRIES})"
                                );
                                if retry_count >= MAX_RETRIES {
                                    state = LteState::Error(ModemError::MqttPublish);
                                    retry_count = 0; // Reset retry count for error state
                        ...
                        retry_count += 1;
                        error!(
                            "[LTE] Failed to serialize trip/GPS data (Attempt {retry_count}/{MAX_RETRIES})"
                        );
                        if retry_count >= MAX_RETRIES {
                            state = LteState::Error(ModemError::Command);
                            retry_count = 0; // Reset retry count for error state
